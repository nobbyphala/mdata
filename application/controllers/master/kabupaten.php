...
class kabupaten extends admin_controller {
    ...
    function get_list_all_kabupaten(){
        echo json_encode($this->db->query("select * from master_kabupaten ORDER BY KABUPATEN_KODE, KABUPATEN_NAMA")->result_array());
    }
    function get_list_kabupaten() {
        echo json_encode($this->db->query("select * from master_kabupaten #where PROVINSI_ID=? ORDER BY KABUPATEN_KODE, KABUPATEN_NAMA", array(intval($this->input->get('id_provinsi'))))->result_array());
    }
