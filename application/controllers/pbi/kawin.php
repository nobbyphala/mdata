...
class kawin extends admin_controller {
    ...
    public $data = array('title' => 'Data PBI Berdasarkan Status Perkawinan');
    ...
    function index(){
    	...
    	$this->load->view('pbi/view_kawin',$this->data);
    }
    function get_chart_column(){
        ...
        $q=$this->db->query("select KAWIN_NAMA AS NAMA from master_kawin order by KAWIN_KODE")->result_array();
        $lhk=array();
        foreach ($q as $key => $row) {
            $lhk[$row['NAMA']]=0;
        }
        $lhk['KATEGORI LAIN']=0;
        $sql="select count(*) as jumlah, coalesce(m.KAWIN_NAMA,'KATEGORI LAIN') AS NAME
                from penduduk p
                left join master_kawin m
                on m.KAWIN_KODE=p.STATUS_KAWIN
                group by NAME";
        if($id_kabupaten>0){
